...
$offset = ($page - 1) * $limit;
$sql = "SELECT s.id, c.country_name ,s.name , s.status FROM states s INNER JOIN countries c ON s.country_id = c.id ";
if (!empty($_GET['search'])) {
    $search = $_GET["search"];

    $sql .= " WHERE country_name LIKE '%$search%'  OR name LIKE '%$search%' ";
} else {
    $search ="";
}
...
?>
    ...
    <header class="page-header">
        <div class="d-flex align-items-center">
            <div class="mr-auto">
                <h1 class="separator">Manage States</h1>
    ...
    <tr>
        <th>Sl no.</th>
        <th><a href="manage_state.php?orderby=country_name&order=<?php echo $CountryName; ?>&limit=<?php echo $limit; ?>&page=<?php echo $page; ?>&search=<?php echo $search; ?>">Country Name</a> <i  class="la <?php echo($orderBy == 'country_name' ? ($order == 'asc' ? 'la-sort-asc' : 'la-sort-desc') : 'la-sort'); ?>"></i></th>
        <th><a href="manage_state.php?orderby=name&order=<?php echo $state; ?>&limit=<?php echo $limit; ?>&page=<?php echo $page; ?>&search=<?php echo $search; ?>">States Name</a> <i  class="la <?php echo($orderBy == 'name' ? ($order == 'asc' ? 'la-sort-asc' : 'la-sort-desc') : 'la-sort'); ?>"></i></th>

        <th><a href="manage_state.php?orderby=status&order=<?php echo $status; ?>&limit=<?php echo $limit; ?>&page=<?php echo $page; ?>&search=<?php echo $search; ?>">Status</a> <i  class="la <?php echo($orderBy == 'status' ? ($order == 'asc' ? 'la-sort-asc' : 'la-sort-desc') : 'la-sort'); ?>"></i></th>
...
if ($count) {
    ...
    <td align="center"><?php echo $serial++; ?></td>
        ...
        <td align="center">
            ...
            <a href="state_status.php?status=<?php echo $status; ?>&id=<?php echo $id; ?>&page=<?php echo $page; ?>"><span
                        class="badge <?php echo $status == "Active" ? "badge-success" : "badge-danger"; ?>"><?php echo $status; ?></span></a>
        </td>
        <td align="right">
            <a href="edit_state.php?id=<?php echo $id; ?>" class="btn btn-success btn-sm"><i class="la la-pencil"></i> Edit </a>
                    ...
                    <ul class="pagination float-right m-0" role="navigation">
                        ...
                        if ($page > 1) { ?>
                            <li class="page-item" aria-disabled="true" aria-label="« Previous"><a
                                        href="manage_state.php?page=<?php echo($page - 1); ?>&limit=<?php echo $limit; ?>&search=<?php echo $search; ?>">
                        ...
                        for ($i = 1; $i <= $total_page; $i++) { ?>
                            ...
                            <li class="page-item <?php echo ($page == "$i") ? 'active' : ''; ?>"
                                aria-current="page"><a
                                        href="manage_state.php?page=<?php echo $i; ?>&limit=<?php echo $limit; ?>&search=<?php echo $search; ?>"><span
